...
export default function ProjectsScreen() {
  ...
  const load = useCallback(async (opts?: { reset?: boolean }) => {
    ...
    const res = await apiGet<{ items: Project[]; meta: { total: number; limit: number; offset: number } }>(
      `/projects?limit=${limit}&offset=${off}&q=${encodeURIComponent(q.trim())}`
    );
    ...
    <View style={{ flex: 1, padding: 12 }}>
      <Row style={{ alignItems: 'center', gap: 8 }}>
        <TextInput
          value={q}
          onChangeText={setQ}
          placeholder="Ara: proje no / adı / firma"
          autoCapitalize="none"
      ...
      {canEdit ? (
        <View style={{ marginTop: 10 }}>
          <PrimaryButton title="+ Yeni Proje" onPress={() => nav.navigate('ProjectForm', { mode: 'create' })} />
        </View>
      ) : null}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#eee' }}>
              <Text style={{ fontSize: 16, fontWeight: '700' }}>{item.project_no} — {item.project_name}</Text>
              <Text style={{ color: '#444', marginTop: 3 }}>{item.company_name}</Text>
              <Row style={{ marginTop: 6, justifyContent: 'space-between' }}>
                <Text style={{ color: '#666' }}>{String(item.project_date).slice(0, 10)} | {item.status}</Text>
                <PrimaryButton title="Aç" onPress={() => nav.navigate('ProjectDetail', { id: item.id })} />
              </Row>
            </View>
          )}

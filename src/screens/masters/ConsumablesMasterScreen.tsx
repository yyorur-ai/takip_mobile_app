...
import type { Consumable } from '../../models/types';

export default function ConsumablesMasterScreen() {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    const res = await apiGet<{ items: Consumable[] }>('/consumables');
    setItems(res.items ?? []);
  }, []);
  ...
  const add = async () => {
    if (!canEdit) return;
    if (!name.trim()) {
      Alert.alert('Eksik', 'Sarf adı girin.');
      return;
    }
    try {
      await apiPost('/consumables', { name: name.trim() });
      setName('');
      await load();
    } catch (e: any) {
  ...
  const edit = async (id: number, newName: string) => {
    if (!canEdit) return;
    try {
      await apiPut(`/consumables/${id}`, { name: newName.trim() });
      await load();
    } catch (e: any) {
      Alert.alert('Hata', e?.message ?? 'İşlem hatası');
    }
  ...
  const del = async (id: number) => {
    if (!canEdit) return;
    Alert.alert('Sil', 'Bu sarfı silmek istiyor musunuz?', [
    ...
    <View style={{ flex: 1, padding: 12 }}>
      {canEdit ? (
        <View style={{ marginBottom: 12 }}>
          <Text style={{ fontWeight: '800', marginBottom: 6 }}>Yeni Sarf</Text>
          <Row style={{ gap: 10 }}>
            <TextInput
              value={name}
              onChangeText={setName}
              placeholder="Sarf adı"
            ...
            {canEdit ? (
              <Row style={{ gap: 10, marginTop: 8 }}>
                <PrimaryButton
                  ...
                  variant="secondary"
                  onPress={() => {
                    Alert.prompt?.('Sarf Adı', 'Yeni adı girin', (text) => {
